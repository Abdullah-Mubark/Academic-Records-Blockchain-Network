...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.academicrecirds.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.academicrecords.com
    docker cp -a orderer.academicrecords.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.academicrecords.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.academicrecords.com

    for PEER in peer0.org1.academicrecords.com peer1.org1.academicrecords.com peer0.org2.academicrecords.com peer1.org2.academicrecords.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/academicrecords-network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/academicrecords-network/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.academicrecords.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.academicrecords.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
